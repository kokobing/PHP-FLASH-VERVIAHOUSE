...
class PageNav{
	...
	//显示信息
	var $go_top="<img src='/PWCS2007/cn/include/img/icon_003.gif' border='0'>";
	var $go_back="<img src='/PWCS2007/cn/include/img/icon_004.gif' border='0'>";
	var $go_next="<img src='/PWCS2007/cn/include/img/icon_005.gif' border='0'>";
	var $go_end="<img src='/PWCS2007/cn/include/img/icon_006.gif' border='0'>";
	/**
	* ���캯��
	...
	function init_page ( $pagesize, $num, $page=false, // 通常指定前三项就 OK
		...
		for($i = max ( 1, $page - $links ); $i <= min ( $this->tpage, $page + $links ); $i++){
			if($page == $i){
				...
				$PageLinks .= "<a href=\"{$this->file}?{$this->varstr}{$f}$var=$i\" class=\"link_01\">$i</a>&nbsp;&nbsp;"; // 其他页的链接
			}
		}
		...
		if ( $this->tpage > 0 ){
			$before	= $page > 1 // 若当前页大于 1， 显示“第一页”
				? "<a href=\"{$this->file}?{$this->varstr}{$f}$var=1\" class=\"link_01\">$txt_fp</a>"
				: "";
			$after	= $page < $this->tpage // 若当前页小于最后页， 显示“最后页”
				? "<a href=\"{$this->file}?{$this->varstr}{$f}$var={$this->tpage}\" class=\"link_01\">$txt_lp</a>"
				: "";

			
				if ( $page > 1 ){ // 若当前页大于 1， 显示“上一页”
					$before	.= " <a href=\"{$this->file}?{$this->varstr}{$f}$var=" . ( $page - 1 ) . "\" class=\"link_01\">$txt_pp</a>";
				}
				if ( $page < $this->tpage ){ // 若当前页小于最后页， 显示“下一页”
					$after	= "<a href=\"{$this->file}?{$this->varstr}{$f}$var=" . ( $page + 1 ) . "\" class=\"link_01\">$txt_np</a> " . $after;
				}

				global $language;
				if( $this->isfull ){
					$after .= $language == "english" ? " $pagesize Records/Page, Total: $num" : "  共{$num}条{$this->tpage}页";
				}
			...
			$PageLinks	= "$strJump1 $PostParam $before $PageLinks $after $strJump2"; 
		}
		if($this->tpage == 0)
		{
			$PageLinks = "";
